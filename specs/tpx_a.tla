...
Models the safety core of the agent flow:
  - budget attestations are single-use at the /token relay (jti replay guard)
  ...
  - the meter is mission-keyed and cumulative across every auth token under
    the mission: reserve worst-case cost before inference, commit or release
    after, never past the bound budget
  - spend requires a funded mission: nothing is reserved or charged until a
    tokenpony user claims the mission ref (402 mission_unfunded)
...
Abstractions: JWS/HTTP-signature verification, token expiry, and the PS relay
handshake are trusted plumbing and not modeled; auth tokens are not tracked
individually because the meter is keyed by mission ref, not token.
"completed" missions gate spend exactly like "revoked" ones and are collapsed
into that phase. The funder's account balance is out of scope; this spec
checks the mission cap only (the OAuth-side spend cap lives in tpx.tla).
*)
EXTENDS Naturals
...
-------------------------------------------------------------------------------
(* The person approves mission m at their Person Server with budget b; the PS
   mints a single-use budget_attestation (seam contract section 4). A PS may
   attest the same mission more than once, with any budget it likes. *)
Approve(a, m, b) ==
  /\ attPhase[a] = "unissued"
...
(* PS -> AS budget relay on /token: the attestation jti is consumed either
   way (replay guard fires before budget binding). The first token binds the
...
(* A tokenpony user claims the mission ref at /fund; open PS federation is
   safe because no money moves before this step. Claim-once: the funder is
   set only while unclaimed and active. *)
Fund(m) ==
  /\ mStatus[m] = "active"
  /\ ~funded[m]
...
(* A metered completion reserves its worst-case cost before inference
   (seam contract section 6). Refused unless the mission is active, funded,
   and the reservation fits under the cap (402 budget_exhausted). *)
Reserve(r, m, k) ==
  /\ reqPhase[r] = "idle"
  /\ mStatus[m] = "active"
  /\ funded[m]
...
(* Inference finished: commit the actual charge, capped at the reservation,
   and release the rest. Deliberately no status check, matching the code: a
   revoke that lands mid-request cannot stop the settle, only bound it. *)
Commit(r, actual) ==
  /\ reqPhase[r] = "inflight"
  /\ actual <= reqAmt[r]
...
(* Revocation by the funder (dashboard) or completion by the PS; both gate
...
(* Every live mission traces to a redeemed attestation, and its bound budget
   is exactly one a Person Server attested for it. *)
